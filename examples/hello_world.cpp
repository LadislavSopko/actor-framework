...
struct stream_msg {
  ...
  stream_id sid;
  variant<open, ack, batch, demand, close> content;
};
...
template <class Inspector>
typename Inspector::result_type inspect(Inspector& f, stream_msg::ack& x) {
  return f(meta::type_name("ack"), x.initial_demand);
}
...
template <class T>
stream_msg make_stream_open(stream_id sid, stream<T> token,
  ...
  return {std::move(sid), stream_msg::open{std::move(msg), pr, po}};
}

inline stream_msg make_stream_ack(stream_id sid, int32_t initial_demand) {
  return {std::move(sid), stream_msg::ack{initial_demand}};
}
...
struct downstream {
public:
  downstream(stream_id sid, strong_actor_ptr successor)
      : data_(new data(std::move(sid), std::move(successor))) {
    CAF_ASSERT(data_->sink != nullptr);
  }
  ...
  void inc_credit(int32_t x) const {
    data_->credit += x;
  }

  void dec_credit(int32_t x) const {
    data_->credit -= x;
  }

  void send_batch(scheduled_actor* self) {
    data_->send_batch(self);
  }

private:
  struct data : ref_counted {
    ...
    data(stream_id&& x0, strong_actor_ptr&& x1)
        : sid(std::move(x0)),
          sink(std::move(x1)),
          credit(0) {
      // nop
    ...
    void send_batch(scheduled_actor* self) {
      ...
      aout(self) << "[id: " << self->id() << "] available credit: "
                 << s << std::endl;
      if (s > 0) {
        auto res = gptr->make_batch(s);
        credit -= res.first;
        aout(self) << "[id: " << self->id() << "] transmit batch: "
                   << res.second << std::endl;
        if (!res.second.empty())
          sink->enqueue(self->ctrl(), message_id::make(),
                              make_message(make_stream_batch(sid, res.first, 
                                                             std::move(res.second))),
                              self->context());
        if (gptr->closed())
          sink->enqueue(self->ctrl(), message_id::make(),
                              make_message(make_stream_close(sid)),
                              self->context());
      }
    }
  };
...
public:
  upstream(stream_id sid, strong_actor_ptr predecessor)
      : data_(new data(std::move(sid), std::move(predecessor))) {
    CAF_ASSERT(data_->source != nullptr);
    CAF_ASSERT(data_->max_credit > 0);
  }
  ...
  int32_t low_watermark() const {
    return data_->low_watermark;
  }

  int32_t assigned_credit() const {
    return data_->assigned_credit;
  }

  int32_t max_credit() const {
    return data_->max_credit;
  }

  void send_demand(scheduled_actor* self, int32_t released_credit) {
    data_->send_demand(self, released_credit);
  }
  ...
  struct data : ref_counted {
    ...
    void send_demand(scheduled_actor* self, int32_t released_credit) {
      ...
      if (assigned_credit < low_watermark) {
        assigned_credit += 10;
        source->enqueue(self->ctrl(), message_id::make(),
                        make_message(make_stream_demand(sid, 10)),
                        self->context());
      }
    }
  };
...
class stream_source_state_visitor {
public:
  using result_type = void;

  stream_source_state_visitor(scheduled_actor* self, src_state& st, stream_id& sid)
      : self_(self),
        st_(st),
        sid_(sid) {
    // nop
  ...
  void operator()(stream_msg::open&) {
    CAF_LOG_ERROR("received stream_open in a source");
  }

  void operator()(stream_msg::ack& x) {
    auto i = st_.data.find(sid_);
    if (i != st_.data.end()) {
      i->second.open_credit = x.initial_demand;
      i->second.transmit(self_);
  ...
  void operator()(stream_msg::batch&) {
    CAF_LOG_ERROR("received stream_batch in a source");
  }
  ...
  void operator()(stream_msg::close&) {
    CAF_LOG_ERROR("received stream_close in a source");
  }
...
behavior source(stateful_actor<src_state>* self) {
  ...
  return {
    [=](add_atom, actor new_sink, std::string& fname) mutable {
      ...
      self->send(new_sink, make_stream_open(sid, stream<int>{}));
      auto& st = self->state;
    ...
    [=](stream_msg& x) {
      debug();
      stream_source_state_visitor v{self, self->state, x.sid};
...
class stream_sink_state_visitor {
public:
  using result_type = void;

  stream_sink_state_visitor(scheduled_actor* self, snk_state& st, stream_id& sid)
      : self_(self),
        st_(st),
        sid_(sid) {
    // nop
  ...
  void operator()(stream_msg::open&) {
    auto upstream = self_->current_sender();
    if (!upstream)
      return;
    auto rp = self_->make_response_promise<stream_msg>();
    auto i = st_.data.find(sid_);
    if (i == st_.data.end()) {
      i = st_.data.emplace(sid_, stream_sink_state{sid_, upstream, 7, 10, 25}).first;
      rp.deliver(make_stream_ack(std::move(sid_), 5));
    } else {
      rp.deliver(sec::invalid_argument);
    }
  }

  void operator()(stream_msg::ack&) {
    CAF_LOG_ERROR("received stream_ack in a sink");
  }

  void operator()(stream_msg::batch& x) {
    auto i = st_.data.find(sid_);
    if (i != st_.data.end())
      i->second.send_demand(self_, x.xs_size);
  }

  void operator()(stream_msg::demand&) {
    CAF_LOG_ERROR("received stream_demand in a sink");
  }
...
behavior sink(stateful_actor<snk_state>* self) {
  return {
    [=](stream_msg& x) {
      debug();
      stream_sink_state_visitor v{self, self->state, x.sid};
      apply_visitor(v, x.content);
    }
  };
}
